/**
 * Profile page — user stats, TDEE breakdown, and Whoop connection.
 */
...
export default function Profile() {
  ...
  useEffect(() => {
    async function checkStatus() {
      try {
        ...
        // backend not reachable — show disconnected
      }
    }
    checkStatus()
  ...
  return (
    <div className="p-4 pb-24">
      ...
      {/* TDEE breakdown — shown once /calories/today has loaded */}
      {breakdown && (
        <div className="bg-[#152A1E] rounded-2xl p-4 border border-[#1E3A2A] mb-4">
          <p className="text-xs text-[#5C8C6E] uppercase tracking-wide font-semibold mb-3">
      ...
      <div className="bg-[#152A1E] rounded-2xl p-4 border border-[#1E3A2A]">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs text-[#5C8C6E] uppercase tracking-wide font-semibold">
            Whoop
          </p>
          <div className={`flex items-center gap-1.5 text-xs font-semibold ${connected ? 'text-green-400' : 'text-[#5C8C6E]'}`}>
            <div className={`w-1.5 h-1.5 rounded-full ${connected ? 'bg-green-400' : 'bg-[#3A5C48]'}`} />
